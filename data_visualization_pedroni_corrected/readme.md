...
I selected a subset of countries such as their writing system belongs to one of these groups:
...
- the **ESCS index** is surely a factor that influences math performance, but..
- ..the **ESCS plotted by language type** suggests that there must be some other factor. In fact, the socioeconomic and cultural conditions seem to have a big impact on performance, but it is not the factor that set language type groups apart (and more, the logographic language group distribution of ESCS index is the one with the lowest mean)


- **English speaking countries and Singapore by ESCS**, make clear that the same language can bring to very different results
  - the schooling system could be a reason  
  - since it is the only Asian country in my English speaking selection, it also suggest a role of cultural background and/or multilingualism.

_All the plots need a title and better axis labels, some also less ink_

## Feedback
The "story" was way too complicate and difficult to understand, since I tried to convey too many findings in a short way.. I have the things more or less clear, after days of looking into my variables, but it is not possible to make them equally clear to others in too short a slideshow.
Also because I use reveal.js and the aesthetic of the slideshow was (and is) not what I really want.
Therefore I simplified removing the exploration of data of some countries that I was trying to use to further support the idea that language type is not that important and ESCS index is not the only factor to consider. But they made things too difficult to follow, without a lot of explanations added.
...
### Language and phonetics, and writing and educational systems of Countries
